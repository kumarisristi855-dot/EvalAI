...
function AddStudents() {
  ...
  useEffect(() => {
    const fetchExamDetails = async () => {
      try {
        const response = await axios.get(`${API_BASE}/api/exam/${examId}/results`);
        setExam(response.data.exam);
        setExistingStudents(response.data.students || []);
      } catch (err) {
        console.error('Error fetching exam details:', err);
        setError('Failed to load exam details. Ensure backend is running.');
      }
    };
    fetchExamDetails();
  ...
  const handleFileChange = (e) => {
    if (e.target.files) {
      ...
      const existingNames = existingStudents.map(s => s.filename);

      selectedFiles.forEach(file => {
        let name = file.name;
        let isDuplicate = existingNames.includes(name) || currentNames.includes(name);
        let finalFile = file;
        ...
        if (isDuplicate) {
          ...
          warnings.push(`File "${name}" already exists. Automatically renamed to "${newName}".`);
        }
  ...
  const handleUploadAndEvaluate = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE}/api/exam/${examId}/upload-students`, data, {
        headers: {
      ...
      await axios.post(`${API_BASE}/api/exam/${examId}/evaluate`);
  ...
  const checkStatus = async () => {
    try {
      const statusResponse = await axios.get(`${API_BASE}/api/exam/${examId}/status`);
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="text-center mb-10">
        <span className="text-xs font-bold text-indigo-400 uppercase tracking-widest">
          {exam ? `${exam.class} • ${exam.subject}` : 'Loading...'}
        </span>
        <h1 className="text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-indigo-200 via-white to-violet-300 mt-1">
          Add More Students
        </h1>
        <p className="text-slate-400 mt-2 text-lg">
          Upload supplementary student PDFs to append to {exam?.name || 'the exam'}.
        </p>
      </div>
        ...
        {!evaluating ? (
            ...
            {files.length > 0 && (
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <span className="text-sm font-semibold text-slate-300">
                  {files.length} student{files.length > 1 ? 's' : ''} ready for upload
                </span>
                <button
                  onClick={() => setFiles([])}
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>Uploading supplementary files...</span>
                </>
              ) : (
                <span>Upload & Evaluate New Students</span>
              )}
            </button>
          </div>
        ) : (
          ...
          <div className="space-y-8 py-6 text-center">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-slate-200">Evaluating New Submissions</h3>
              <p className="text-slate-400 text-sm">
                Grading {progress.evaluated} of {progress.total} new students...
              </p>
            </div>
            ...
            <p className="text-xs text-slate-500 italic max-w-sm mx-auto">
              Only the new submissions are being evaluated. Existing student grades are not re-calculated.
            </p>
          </div>
        )}
      </div>
