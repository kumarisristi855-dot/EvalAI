...
async function runEvaluation(examId) {
  try {
    ...
    const students = db.prepare("SELECT * FROM students WHERE exam_id = ? AND status = 'pending'").all(examId);
    ...
    for (const question of questions) {
      ...
      const getAnswersStmt = db.prepare(`
        SELECT e.id as evaluation_id, e.student_id, e.student_answer, s.student_name
        FROM evaluations e
        JOIN students s ON e.student_id = s.id
        WHERE e.question_id = ? AND e.student_id IN (${placeholders})
      `);
      
      const rawAnswers = getAnswersStmt.all(question.id, ...studentIds);

      // Programmatic MCQ Grading & SQLite Update
      if (question.question_type === 'mcq') {
        const updateMcqStmt = db.prepare(`
          UPDATE evaluations 
          SET marks_awarded = ?, feedback = ?
          WHERE student_id = ? AND question_id = ?
        `);

        const mcqTransaction = db.transaction(() => {
          for (const ra of rawAnswers) {
            ...
            updateMcqStmt.run(marksAwarded, isCorrect ? 'Correct' : 'Incorrect', ra.student_id, question.id);
          }
        });
      ...
      for (let i = 0; i < studentAnswersForEvaluator.length; i += batchSize) {
        ...
        // Store graded marks and feedback inside SQLite
        const updateEvalStmt = db.prepare(`
          UPDATE evaluations 
          SET marks_awarded = ?, feedback = ? 
          WHERE id = ?
        `);
    ...
    const updateStudentStmt = db.prepare(`
      UPDATE students
      SET total_marks_obtained = ?, percentage = ?, grade = ?, status = 'evaluated'
      WHERE id = ?
    `);
    ...
    const gradeTransaction = db.transaction(() => {
      for (const student of students) {
        const scoreResult = selectTotalScoreStmt.get(student.id);
        const rawScore = scoreResult.total_score || 0;
